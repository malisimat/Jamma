...
- Human decision: accepted ([decision](decisions.md#findings)).
- Phase 4 execution: characterization `b379f26` first built successfully and then exposed both ownership edges as 0/2 expected failures: final-take removal cleared timing on `OnTick`, while a connected empty job visit discarded accepted remote timing. Implementation `bcfa055` removes take counting and timing cleanup from `OnTick`; the job-owned Scene visit now uses immutable take snapshots, preserves connected remote timing, and consumes each disconnected-empty cleanup edge once. P8 passed 1/1, `SceneReset.*` passed 4/4, the focused filter passed 103/103, both edge tests passed 20 repeated runs, and Trigger plus SceneReset passed 21/21. Only Scene and its focused test changed. The pre-existing deferred-trigger snapshot allocation/vector mutation remains callback-reachable outside B008, and live/race/sanitizer/profiler evidence remains for Stage 21. Independent B008 review is pending.
