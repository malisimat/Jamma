...
**B004 sequencing addendum (approved against `HEAD` `0aee948d3af6b4e9bde7103dcfd53d9953f4b5b7`):** The human authorizes a characterization-first exception for B004: commit/register P5/P6, record their expected pre-fix failures, implement strictly inside the existing B004 boundary, then require both tests green before advancing. The same ordering adjustment is permitted for a later batch only when a truthful prerequisite necessarily depends on that batch's already-approved production behavior: its pre-fix failure must be recorded first, implementation remains bounded, and all focused verification must be green before independent approval or dependent work. All original protected timing invariants, prohibited collateral, rollback rules, dependencies, and no-batch residual decisions remain unchanged.

**B006+B007 atomic-batch addendum (approved against `HEAD` `9431c3f859261f6581c4e188988d987a4be3cca4`):** The human authorizes B006 and B007 to execute and receive independent review as one atomic batch. Their owned files, prerequisites P1/P2/P3, rollback requirements, and verification contracts are combined because complete-state production and consumption cannot form a green single-authority intermediate state separately. Both original prohibited-collateral sets, all protected timing invariants, downstream gates, and no-batch residual decisions remain in force. B008 cannot begin until `batch-reviews/B006-B007.md` approves the full combined range.
