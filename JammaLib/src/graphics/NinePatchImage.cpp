...
std::unordered_map<std::string, NinePatchImage::BorderInfo> NinePatchImage::_cachedBorders = {};
	...
	if (validated)
	{
		...
		else
		{
			...
			else
			{
				...
				if (!border.has_value())
				{
					std::cout << "NinePatchImage: texture '" << _drawParams.Texture
						<< "' has no magenta marker pixel (BGRA=255,0,255,255) and no cached border" << std::endl;
					validated = false;
				}
				else
				{
...
bool NinePatchImage::_InitTexture(ResourceLib& resourceLib)
{
	...
	{
		std::cout << "NinePatchImage: missing texture resource '" << _drawParams.Texture << "'" << std::endl;
		return false;
	}
	...
	if (!resource)
	{
		std::cout << "NinePatchImage: expired texture resource '" << _drawParams.Texture << "'" << std::endl;
		return false;
	}

	if (TEXTURE != resource->GetType())
	{
		std::cout << "NinePatchImage: resource type mismatch for texture '" << _drawParams.Texture << "'" << std::endl;
		return false;
	}
...
bool NinePatchImage::_InitShader(ResourceLib& resourceLib)
{
	...
	{
		std::cout << "NinePatchImage: missing shader resource '" << _ninePatchParams.Shader << "'" << std::endl;
		return false;
	}
	...
	if (!resource)
	{
		std::cout << "NinePatchImage: expired shader resource '" << _ninePatchParams.Shader << "'" << std::endl;
		return false;
	}

	if (SHADER != resource->GetType())
	{
		std::cout << "NinePatchImage: resource type mismatch for shader '" << _ninePatchParams.Shader << "'" << std::endl;
		return false;
	}
...
bool NinePatchImage::_InitVertexArray()
{
	...
	const auto uBounds = _BuildBounds(_borderX, _texWidth);
	const auto vBounds = _BuildBounds(_borderY, _texHeight);
...
std::array<GLfloat, 162> NinePatchImage::BuildPositions(
	...
	Size2d size)
{
	const auto xBounds = _BuildBounds(borderX, size.Width);
	const auto yBounds = _BuildBounds(borderY, size.Height);
...
std::array<unsigned int, 4> NinePatchImage::_BuildBounds(unsigned int border, unsigned int extent)
{
