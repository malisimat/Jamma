...
| Applied desired-state receipt | Audio callback after accepted boundary application | Job logger through `Scene::_LogAppliedNinjamLoopAlignment` | Atomic odd/even complete epoch/version/generation/intent/policy/scene/delta receipt | AudioHost member; writer hot | B006+B007 provides desired-versus-applied correlation; bounded diagnostic formatting remains B014 |
...
| Published station membership | Scene publishes after mutations at `Scene.cpp:2147`–`:2301` | AudioHost/Scene audio paths; job-owned empty-state detection uses per-station take snapshots | Immutable `shared_ptr<const vector<shared_ptr<Station>>>`, release/acquire | Snapshot pins stations; hot | B008 `bcfa055` removes callback empty-reset traversal and uses immutable take snapshots on the job visit; independent review pending |
| AudioHost policy/common map | Audio callback desired-versus-applied transaction | Audio callback | Epoch/`NoSync` clears authority without moving cursors; replacement/discipline applies coherently, while the existing per-take map still awaits B009 consolidation | AudioHost member; hot | B006+B007 closes complete-state ordering; one-common-map calculation remains F-006/B009 |
| Per-take map/anchors/cursors | Audio callback through Station epoch-reset and timing fan-out | Audio plus atomic diagnostic/UI readers | Epoch reset clears NINJAM generation/map/anchors only; queued local corrections remain independent; immutable membership snapshots | Take pinned by Station snapshot; hot | Corrected P3 at `da0db24` proves real `M`/`2M`/`3M` audio/MIDI/automation free-run, fresh generation 1, and stale/equal rejection; independently approved `b3f2beb` |
...
| Coordinator/tracker/options | Job/UI under `_sceneMutex` | Job/UI | Plain compound state machine under intended owner | Network service owned; non-audio | B008 `bcfa055` removes the empty-scene callback read of `HasConnectedTiming`; live/race evidence remains Stage 21 |
| Remote MIDI grid | Coordinator creates a grid value paired with each accepted desired geometry; Scene forwards it to Quantiser | Audio quantisation reader; job owner performs empty-state cleanup | Producer-paired geometry/generation/origin; Quantiser publishes its atomic odd/even complete snapshot | Take lifetime; reader hot | Corrected P2 proves desired/grid coherence; B008 preserves connected-empty timing and performs disconnected clear once off callback, pending independent review |
