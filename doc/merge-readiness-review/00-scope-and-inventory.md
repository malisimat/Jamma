...
| Alignment diagnostics | Coordinator job owner captures rejection and desired/applied transitions | Scene job path presents only when Event logging is verbose | Fixed 32-event ledger plus overflow/per-reason counters; diagnostic state remains separate from operational timing | Disabled capture returns before diagnostic work; no callback formatter, I/O, hierarchy traversal, or dead receipt | B014 `058c486` implemented and focused-verified; independent review and live trace pending |
