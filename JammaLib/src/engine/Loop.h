	...
	public:
		...
		Loop& operator=(Loop&& other)
		{
			if (this != &other)
			{
				...
				_loopLength.store(other._loopLength.load(std::memory_order_relaxed), std::memory_order_relaxed);
				other._loopLength.store(loopLength, std::memory_order_relaxed);
				...
				other._writeIndex.store(writeIndex, std::memory_order_relaxed);
				auto playIndex = _playIndex.load(std::memory_order_relaxed);
				_playIndex.store(other._playIndex.load(std::memory_order_relaxed), std::memory_order_relaxed);
				other._playIndex.store(playIndex, std::memory_order_relaxed);
				auto playState = _playState.load(std::memory_order_relaxed);
				_playState.store(other._playState.load(std::memory_order_relaxed), std::memory_order_relaxed);
				other._playState.store(playState, std::memory_order_relaxed);
