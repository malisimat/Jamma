...
- Before every build or native-test run, read the local `.vscode/tasks.json` when it exists. It is the authoritative machine-specific source for the installed build tools, their locations, and explicit build commands. Run the applicable task's MSBuild executable and arguments directly rather than assuming an MSBuild location, Visual Studio edition, shell environment, or PATH configuration.
