...
- Dependencies: F-009 owner/publication contract; approved F-024 complete-state direction; F-021 coherent upstream observation; F-023/F-029/F-030 coherent wide local observation; F-025/F-027 epoch; F-026 observation instant; F-028 validity/loss; F-031 conversion bound. The Phase 2 command-order and two-session reconnect regressions are the required focused contracts; use the Phase 4 passing-first default or characterization-first exception as applicable.
...
- Stage 14: provide one or two focused prerequisite contracts for each major refactor, using the Phase 4 passing-first default or characterization-first exception as applicable. Highest priority remains complete-state coalescing and two-session reconnect. S13-03 additionally needs unequal lengths/offsets/rebase; S13-05 needs caller-gate characterization.
