...
Pending. Phase 3 is complete and stopped at this human gate. Stages 19–21, cleanup batching, source/test implementation, build/test execution, and `merge-brief.md` remain unauthorized.

After recording G3-1–G3-10, the finding/refinement outcomes, explicit compatibility/security/log-policy choices, accepted residual risks, and notes in `../decisions.md`, use this prompt:

> Phase 3 of `doc/merge-readiness-review/merge-readiness-plan.md` is complete. I have recorded all G3-1–G3-10 decisions, F-005/F-006/F-009/F-021/F-024/F-027/F-032/F-034 Phase 3 refinement outcomes, F-035–F-050 outcomes, compatibility/security/log-policy choices, and accepted residual risks in `doc/merge-readiness-review/decisions.md`. Execute Phase 4 reconciliation through its proposed cleanup-batch human gate only: run bounded non-colliding Stages 19–21 in planning/design mode; reconcile all phase packets and canonical findings; produce `stage-reports/19-reconciliation.md`, `20-change-impact.md`, and the design section of `21-merge-hygiene.md`; propose dependency-ordered batches in `cleanup-backlog.md` with one or two passing prerequisite tests per major refactor, exact owners/prohibited collateral/rollback/verification; produce `phase-packets/phase-4.md`; preserve the protected timing glossary and timing-only cleanup scope; do not implement cleanup, run cleanup verification, create batch reviews, or write `merge-brief.md`; stop at the batch-approval human gate. Add separate git commits for each stage, canonical reconciliation, and proposed batch.
