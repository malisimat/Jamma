...
The suite below deliberately selects **one or two passing prerequisites before each major refactor**, as required by the human gate. Tests should be landed and observed passing in a separate Stage/Finding commit before the corresponding production refactor; existing strong helper tests remain supporting evidence rather than being copied into new simulations.
